...
export function getSortedTimeZones() {
  ...
  return Intl.supportedValuesOf("timeZone")
    .map((tz) => {
      ...
      // Extract GMT offset string like GMT-07:00
      const offsetMatch = shortName.match(/GMT([+-]\d{2}):?(\d{2})?/);
      const offsetStr = offsetMatch
        ? `GMT${offsetMatch[1]}:${offsetMatch[2] || "00"}`
        : "GMT";

      const city = tz.split("/").pop().replace(/_/g, " ");

      const label = `${offsetStr} - ${longName} (${shortName}) - ${city}`;
    ...
    .filter(Boolean)
    .sort((a, b) => a.label.localeCompare(b.label)); // alphabetically sort full label
}
*/